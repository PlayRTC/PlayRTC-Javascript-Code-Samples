...
var Peer = utils.Extend(utils.Event, {
	...
	replacePreferCodec: function(sdp, mLineReg, preferCodec){
		...
		if(preferCodec === "H264"){
			//sdp = sdp.replace("packetization-mode=1", "packetization-mode=0");
		}
	...
	createOffer: function(){
		...
		this.pc.createOffer(utils.bind(function(sessionDesc){
			sessionDesc.sdp = this.replaceBandWidth(sessionDesc.sdp, "H264");
			sessionDesc.sdp = this.replacePreferCodec(sessionDesc.sdp, /m=audio(:?.*)?/, "ISAC");
			sessionDesc.sdp = this.replacePreferCodec(sessionDesc.sdp, /m=video(:?.*)?/, "H264");
